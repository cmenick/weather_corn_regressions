...
```{r}
winneshiek <- left_join(winneshiek,winnesummer,by="year") %>% 
  drop_na()
```

```{r}
lm_winne3 <- lm(yield ~ meantmax, winneshiek)
winneshiek$lm3fit <- lm_winne3$fitted.values

ggplot(winneshiek,aes(meantmax,yield)) +
  geom_point() +
  geom_line(mapping = aes(x = meantmax, y = lm3fit)) +
  theme_bw()
```
```{r}
lm_winne4 <- lm(yield ~ meantmax + year, winneshiek)
winneshiek$lm4fit <- lm_winne4$fitted.values

ggplot(winneshiek,aes(meantmax,yield)) +
  geom_point() +
  geom_line(mapping = aes(x = meantmax, y = lm4fit)) +
  theme_bw()
```
